...
export class TemplatesController {
  ...
  async createTemplate(@Body() createTemplateDto: CreateTemplateDto, @Req() request: any, @Res() response: Response) {
    const { status, ...restOfResponse } = await this.templatesService.createTemplate(request.user.id, request.teamId, createTemplateDto);
    response.status(status).json(restOfResponse);
  }

  @Get()
  async getTemplates(@Query() filter: TemplateFilterDto, @Req() request: any, @Res() response: Response) {
    const { status, ...restOfResponse } = await this.templatesService.getTemplates(request.user.id, request.teamId, filter);
    response.status(status).json(restOfResponse);
  }

  @Get(':id')
  async getTemplateDetails(@Param('id') id: string, @Req() request: any, @Res() response: Response) {
    const { status, ...restOfResponse } = await this.templatesService.getTemplateDetails(id, request.user.id, request.teamId);
    response.status(status).json(restOfResponse);
  }

  @Put(':id')
  async updateTemplate(@Param('id') id: string, @Body() updateTemplateDto: UpdateTemplateDto, @Req() request: any, @Res() response: Response) {
    const { status, ...restOfResponse } = await this.templatesService.updateTemplate(id, request.user.id, request.teamId, updateTemplateDto);
    response.status(status).json(restOfResponse);
  }

  @Delete(':id')
  async deleteTemplate(@Param('id') id: string, @Req() request: any, @Res() response: Response) {
    const { status, ...restOfResponse } = await this.templatesService.deleteTemplate(id, request.user.id, request.teamId);
    response.status(status).json(restOfResponse);
  }

  @Post(':id/duplicate')
  async duplicateTemplate(@Param('id') id: string, @Req() request: any, @Res() response: Response) {
    const { status, ...restOfResponse } = await this.templatesService.duplicateTemplate(id, request.user.id, request.teamId);
    response.status(status).json(restOfResponse);
  }
